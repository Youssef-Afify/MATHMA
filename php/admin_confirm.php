<!-- Enter password before editing admin's info -->
<?php
include "db.php";
...
} elseif ($_GET['adminid'] != $_SESSION['admin_id']) {
...
<body>
    ...
    if (isset($_POST['confirmed'])) {
        $confirmedpass = $_POST['confirmedpassword'];
        if (password_verify($confirmedpass, $_SESSION['admin_password'])) {
            $temp = $_SESSION['admin_id'];
            header("location: admin_edit.php?adminid=$temp");
        } else {
            echo "<div class='mx-auto w-50 alert  alert-danger alert2 dm-serif-text-regular-italic' role='alert'>Wrong Password</div>";
            style();
    ...
    <div class="container my-3" id="confirm">
        <div class="container">
            <h1 class="dm-serif-text-regular-italic desc" style=""><br>Enter your password</h1>
            <br><br>
            <form action="" method="post">
                <div class="form-floating my-4">
                    <input type="password" class="form-control" id="floatingPassword" name="confirmedpassword" required
                        placeholder="Password">
                    <label for="floatingPassword">Password</label>
                </div>
                <div>
                    <input class="mb-4 w-100 dm-serif-text-regular-italic" id="mysub" type="submit" name="confirmed"
                        value="Confirm">
                </div>
            </form>
        </div>
